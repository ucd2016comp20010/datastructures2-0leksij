...
public class AVLTreeMap<K, V> extends TreeMap<K, V> {
    ...
    protected Position<Entry<K, V>> tallerChild(Position<Entry<K, V>> p) {
        ...
        // tie-break: return the child on the same side as p relative to its parent
    ...
    protected void rebalance(Position<Entry<K, V>> p) {
        ...
        do {
            oldHeight = height(p);
            if (!isBalanced(p)) {
                // perform trinode restructure rooted at taller grandchild
...
//I had to add this, because toString and Remove K tests basically need opposite to string methods.
    public String toStringO() {
        ...
        for (Position<Entry<K, V>> p : tree.inorder()) {
            if (isInternal(p)) {
                if (!first) sb.append(", ");
                sb.append(p.getElement()); // MapEntry.toString() returns just the key
                first = false;
            }
        }
        sb.append("]");
