...
public abstract class AbstractTree<E> implements Tree<E> {
    ...
    public int height_recursive(Position<E> p) {
        int h = 0;
        for (Position<E> child : children(p)) h = Math.max(h, height_recursive(child));
    ...
    private void postorderSubtree(Position<E> p, List<Position<E>> snapshot) {
        for (Position<E> child : children(p)) postorderSubtree(child, snapshot);
        snapshot.add(p);
    }
    ...
    public Iterable<Position<E>> breadthfirst() {
        ...
        while (!queue.isEmpty()) {
            ...
            snapshot.add(p);
            for (Position<E> child : children(p)) queue.add(child);
        }
        return snapshot;
    }
